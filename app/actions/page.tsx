...
const actions = [
  ["01", "デジタルハザードマップ開発", "地域の危険箇所、避難所、避難ルートをマップ上に整理。現在は、危険箇所の報告や周辺避難所、気象・SNS情報の確認、ルート作成などを一つの地図で扱えるアプリを開発しています。", "/assets/hazard-map-app-development.png", ["危険箇所の可視化", "避難ルートの共有", "地域参加型の更新"]],
      ...
      <section className="action-detail-list">
        {actions.map(([number, title, text, image, points]) => (
          <article className="action-detail" id={`action-${number}`} key={number as string}>
            <div className={`action-detail-image${number === "01" ? " is-app-screen" : ""}`}>
              <img src={withBasePath(image as string)} alt={number === "01" ? "開発中のデジタルハザードマップアプリ画面" : ""} />
              {number === "01" && <small>DEVELOPMENT SCREEN / 開発中のアプリ画面</small>}
              <span>#{number}</span>
            </div>
